% Surface detection
% Depth index matrix
SURF=flipdim(repmat(uint16((0:size(M,1)-1))',[1 size(M,2) size(M,3)]),1); %inverte le colonne della matrice dal basso vs l'alto
SURF(M<=tresh)=0;
surf=squeeze(max(SURF)); %Remove singleton dimensions
clear SURF

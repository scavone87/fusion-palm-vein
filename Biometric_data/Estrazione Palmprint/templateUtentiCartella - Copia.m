...
s = 'Selezionare la directiory contenente le sottocartelle contenenti le immagini jpg (imageGeneratedFrom3D)';
elencaSottocartelle;
...
for k=1:sizeSubFolders
    ...
    for i=3:sizeFilesJpg %salto le prime due a profondità 0.05 e 0.1 perche' le ho fuse nella 3.5
        ...
        switch choice
            ...
            case 'DoG'
            [fileDat,outputImage]= DoGfilter(fullPathName);
        ...
        dlmwrite([saveDir '\' folderSaveName '/' utenteCorrente '\'  'TEMPLATE', num2str( i+1 ), '.dat'],fileDat);
%         imwrite(outputImage,[saveDir '\' folderSaveName '/' utenteCorrente '\' 'TEMPLATE', num2str( i+1 ), '.jpg']);
        imwrite(fileDat,[saveDir '\' folderSaveName '/' utenteCorrente '\' 'TEMPLATE', num2str( i+1 ), '.jpg']);
        step=step+1;
